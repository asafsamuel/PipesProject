...
public class DesignTest {
	...
	public void testDesign4test(){
		testImp(serverInteface,serverClass,2);
		testImp(clientHandlerInterface,clientHandlerClass,2);
		testImp(cacheManagerInterface,cacheManagerClass,2);
		testImp(solverInterface,solverClass,2);
		int i=0;
		boolean ar[]={false,false};
		for( Field f : clientHandlerClass.getDeclaredFields()){
			if(f.getType().equals(solverInterface) || f.getType().equals(cacheManagerInterface)){
		...
		testImp(searchableInterface,pipeGameClass,5);
		testImp(searcherInterface,bestFSClass,5);
